...
export function useVideoElement() {
  ...
  const getOrCreateVideoElement = useCallback(
    (
      ...
      if (!videoElement || !document.body.contains(videoElement)) {
        console.log(
          `[useVideoElement] Создаем новый видео элемент для ${video.id}`,
        );
        ...
        console.log(
          `[useVideoElement] Используем существующий видео элемент для ${video.id}`,
        );
      }
  ...
  const updateVideoSrc = useCallback(
    (videoElement: HTMLVideoElement, video: MediaFile) => {
      // Проверяем, что src установлен правильно
      if (videoElement && !videoElement.src?.includes(video.id) && video.path) {
        console.log(
          `[useVideoElement] Обновляем src для видео ${video.id}: ${video.path}`,
        );
        videoElement.src = video.path;
        videoElement.load();
      }
  ...
  const cleanupUnusedVideoElements = useCallback(
    (activeVideoIds: string[], videoRefs: Record<string, HTMLVideoElement>) => {
      ...
      const unusedVideoIds = allVideoIds.filter(
        (id) => !activeVideoIds.includes(id),
      );
      ...
      unusedVideoIds.forEach((id) => {
        ...
        if (videoElement && document.body.contains(videoElement)) {
          console.log(
            `[useVideoElement] Удаляем неиспользуемый видео элемент: ${id}`,
          );
