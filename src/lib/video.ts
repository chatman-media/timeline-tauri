...
export const calculateTimeRanges = (videos: MediaFile[]): TimeRange[] => {
  ...
  const times = videos.flatMap((v: MediaFile) => {
    const startTime =
      (v.startTime ?? 0) * TIME_CONSTANTS.MILLISECONDS_IN_SECOND;
    const duration = v.duration ?? 0;
    const endTime =
      startTime + duration * TIME_CONSTANTS.MILLISECONDS_IN_SECOND;
    // Отключаем логирование для уменьшения количества сообщений
    // console.log(
...
export const getAspectRatio = (stream?: FfprobeStream): string | null => {
  ...
  if (stream.sample_aspect_ratio && stream.width && stream.height) {
    const [sar_num, sar_den] = stream.sample_aspect_ratio
      .split(":")
      .map(Number);
